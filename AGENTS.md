...
- **`library/`** — `@tumaet/apollon`, the embeddable React editor, published to npm. Native HTML + Radix UI primitives (`@radix-ui/react-popover`, `@radix-ui/react-tooltip`) styled via `--apollon-*` CSS variables; `lucide-react` icons; Yjs collaboration — the live-cursor/presence layer lives here ([`collaboration.md`](docs/library/api/collaboration.md)).
