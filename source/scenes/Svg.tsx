...
const normalizeState = (state: ModelState): Element[] => {
  let elements: Element[] = state.diagram.ownedElements.map(id => state.elements[id] as Element);
  // TODO
  ...
  const relationships: Relationship[] = state.diagram.ownedRelationships.map(id => state.elements[id] as Relationship);
  // .map(element => Object.setPrototypeOf(element, (Plugins as any)[element.type].prototype));
