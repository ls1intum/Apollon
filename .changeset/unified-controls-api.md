...
- **Grid-framed bands.** The header/footer/rail bands are laid out in a CSS-grid frame over the full-bleed canvas (header/footer own the top/bottom rows, rails own the middle-row side columns), so bands are structurally incapable of overlapping each other on any screen size — no more palette-over-header collision. Device safe-area and mobile-keyboard insets are handled once as grid padding.
