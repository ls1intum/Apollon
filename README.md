...
- **[`standalone/`](./standalone)** — standalone web app (server + app) built on the library.
...
- App (Vite HMR) on a printed local dev URL.

The launcher handles the boring parts:

- Resolves port collisions for the app, server, WebSocket relay, and Redis.
...
Override ports via `APOLLON_APP_PORT`, `APOLLON_SERVER_PORT`, `APOLLON_WS_PORT`, or `APOLLON_REDIS_PORT`.
...
| App        | React, TypeScript, Vite, MUI, Tailwind                               |
