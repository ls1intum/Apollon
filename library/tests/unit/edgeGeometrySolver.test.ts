...
describe("computeAllEdgeGeometry — auto anchor optimization", () => {
  ...
  it("merges a UML generalisation fan into ONE arrowhead", () => {
    // Several sub-classes inheriting one super-class share a single hollow triangle
    // (Purchase's UML studies rank joined inheritance arcs with crossings and bends).
    // This is the ONE case where overlaying edges is correct, so it is opt-in per edge
    // type — an association fan on the same side must still spread.
    const S = makeNode("super", 200, 0, 160, 100)
    const A = makeNode("aaa", 0, 250, 160, 100)
    const B = makeNode("bbb", 200, 250, 160, 100)
    const C = makeNode("ccc", 400, 250, 160, 100)
    const inherit = computeAllEdgeGeometry(
      base(
        [S, A, B, C],
        [
          {
    ...
    const heads = ["i1", "i2", "i3"].map((id) => inherit[id].at(-1)!)
    for (const h of heads) {
      expect(h.x).toBeCloseTo(heads[0].x, 5)
      expect(h.y).toBeCloseTo(heads[0].y, 5)
    }

    // The same fan of ASSOCIATIONS must NOT merge — each keeps its own port.
    const assoc = computeAllEdgeGeometry(
      base(
        [S, A, B, C],
        [
          {
    ...
    const aHeads = ["a1", "a2", "a3"].map((id) => assoc[id].at(-1)!)
    const distinct = new Set(aHeads.map((h) => `${h.x},${h.y}`))
    expect(distinct.size, "association fan must stay spread").toBe(3)
  })
})
