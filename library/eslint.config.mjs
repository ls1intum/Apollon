...
export default [
  ...
  // ESLint 10 release (peer caps at `^9.7`; the v10 RuleContext fix is an
  // unmerged PR). `recommended-typescript` covers the old plugin's recommended
  // set (jsx key, children/danger misuse, deprecated/legacy lifecycles, direct
  // state mutation, …) and disables the prop-types rules TypeScript already
  // enforces — so the previous `react/prop-types: off` is no longer needed. It
  // also self-detects the React version, replacing `settings.react`.
