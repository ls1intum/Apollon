Privacy Statement for the Apollon web, iOS, and Android apps in accordance with
Art. 13 GDPR.
...
the browser tab or app session. By design, the service keeps no per-request
access logs: your IP address and user-agent are not persistently recorded. Only
operational events about the service itself (certificate renewals, critical
errors) are captured, and they contain no personal data.
...
Apollon is a UML modelling editor available in a browser and as a mobile app.
You draw diagrams locally and can optionally share them via a link. Shared
diagrams are stored on TUM infrastructure in a database hosted in Germany and
are removed automatically **120 days** after the last write. Real-time
collaborative edits are relayed over a WebSocket connection while the diagram is
open; the server holds those messages in memory only and writes nothing beyond
the database record.
...
**Live-collaboration data.** Only if you have entered a display name in the _Collaborate_ dialog: your display name, cursor position, and current selection. As described in §1, the server holds these in a per-diagram in-memory record while you are connected and forwards them to your collaborators in real time. Nothing is written to disk.

**Request metadata (IP address and user-agent).** Transmitted automatically by
your browser or app on every HTTP request and visible to TUM's servers only for
the duration of the request, so the response can be routed back to your client.
...
_Legal basis — client-side storage (theme, draft diagrams, optional
collaboration name; see §3): § 25(2) no. 2 of the German Telecommunications
Digital Services Data Protection Act (TDDDG). Each entry is strictly necessary
for a service you have explicitly requested, so no consent is required._
...
- **Your locally drafted diagrams**
  (`localStorage["persistenceModelStore"]`) — the diagrams you are currently
  authoring. Kept on your own device so you do not lose your work between
  visits. Sent to TUM only when you actively click _Share_. Persists until you
  delete the diagram, clear its storage, or uninstall the app.
- **Your collaboration display name**
  (`sessionStorage["apollon-collab-name"]`) — only set if you have opened the
  _Collaborate_ dialog. Discarded automatically when you close the browser tab
  or app session. The name itself is also sent to TUM and shown to your
  collaborators while a session is open (see §1 and §2).

You can remove these entries by deleting diagrams or clearing the browser or app
storage.

_Legal basis: § 25(2) no. 2 TDDDG. Each entry is unconditionally necessary to deliver the service you have requested; no consent is required._
...
**Other end users — live collaboration.** If you have opened the _Collaborate_ dialog and entered a display name, the other Apollon users who have joined the same diagram and entered a display name see your name, cursor position, and current selection in real time while you are connected. They are end users of the same TUM-operated service, not external recipients.
...
| Theme preference and locally drafted diagrams                     | Until you delete them, clear browser/app storage, or uninstall the app                  | Your own device                                     |
| Optional collaboration display name                               | Until you close the browser tab or app session                                          | Your own device (and server memory while connected) |
...
Use of Apollon is voluntary. You are under no statutory or contractual
obligation to provide data, and declining to use the service has no
consequences. Your IP address and user-agent are transmitted automatically by
your browser or app when it makes a network request; this is technically
unavoidable for online sharing and collaboration.
