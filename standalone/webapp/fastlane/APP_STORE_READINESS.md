# App Store release readiness

The repository contains the repeatable parts of an App Store release:

- the existing App Store bundle identifier, `de.tum.cit.ase.apollon`;
- the legacy Apollon app icon and launch-screen design;
- localized public metadata in `fastlane/metadata`;
- real iPhone and iPad Simulator screenshots generated by XCUITest;
- Fastlane lanes for TestFlight, assets-only updates, and promotion of the
  tested TestFlight build to the App Store; and
- a manual GitHub Actions workflow whose App Store review submission is opt-in.

## Before uploading

- Merge and deploy the documentation before running an App Store destination.
  The release workflow validates the marketing, support, and privacy URLs and
  stops before any upload if one is not a public HTTPS page.
- Install an App Store build of the old native app, add representative diagrams,
  then update it in place with the new build. Verify the legacy migration on an
  iPhone and an iPad and export a backup before testing. See
  `ios/App/App/Migration/README.md`.
- Open `fastlane/screenshots-review.html` and review both the untouched
  Simulator masters and framed candidates at full size. Confirm the upload
  style in the release workflow.
- If using the framed set, have an authorized Apple Developer Program
  representative accept Apple's product-bezel license and install the current
  official assets under the ignored `fastlane/device-frames/` directory. The
  release lanes fail closed rather than substituting unofficial frames.
- CI fetches the approved, checksum-pinned packages directly from Apple and
  publishes a screenshot review artifact before uploading. A changed package
  fails validation before its bundled terms are accepted.
- Verify the version and build number, signing certificate, provisioning profile,
  distribution agreements, tax and banking status, and DSA trader status in App
  Store Connect.
- The App Review contact is uploaded from the `IOS_APP_REVIEW_*` CI secrets (see
  **App Review Information**); populate them once so no personal data is committed.

## App Store Connect answers

Almost every App Store Connect field is now set **code-first by the release
lanes** — categories, the age-rating questionnaire, export compliance, content
rights, and (from CI secrets) the App Review contact. The single exception is
the App Privacy "nutrition label," which Apple exposes only through a
session-authenticated action, not the API key (see below). Each answer reflects
the shipped app: exports write to the app's own cache and hand off through the
iOS share sheet; only the Share/Collaborate features transmit anything off
device; there are no accounts, analytics, or tracking (the only cookie is the
strictly-necessary diagram-owner token set when sharing), and the IP
address/user-agent are not retained. Confirm each still matches production
behavior before the first submission.

The values below document what the lanes submit (and, for App Privacy, what to
enter):

### App Privacy ("nutrition label")

- **Data is collected — Yes**, and only through the optional Share/Collaborate
  feature. Declare exactly one data type:
  - **User Content → Other User Content** — the diagram the user chooses to
    share, plus the display name they type if they open the Collaborate dialog.
    - Linked to the user's identity? **No** (no accounts, no identifiers).
    - Used for tracking? **No.**
    - Purpose: **App Functionality** only.
- **Everything else: Data Not Collected.** Explicitly NOT collected: Contact
  Info, Health & Fitness, Financial Info, Location, Sensitive Info, Contacts,
  Browsing/Search History, Identifiers, Purchases, Usage Data, Diagnostics. The
  IP address and user-agent are visible only transiently to route each response
  and are never retained, so under Apple's definition they are not "collected."

Shared diagram content is stored server-side only when the user shares it, and is
auto-deleted 120 days after the last edit (stated in the privacy policy).

**Automation:** the nutrition label is the one field the App Store Connect API
(and therefore `deliver`) cannot set. The separate `privacy` lane uploads the
reviewed, version-controlled `fastlane/app_privacy_details.json` declaration.
That action needs an **Apple ID session** (`FASTLANE_USER` +
`FASTLANE_SESSION`), not the API key. Confirm the JSON still matches the
declaration above, then run `bundle exec fastlane privacy`. Publishing privacy
answers remains a deliberate, app-level legal action rather than part of every
version upload.

### Age rating

**Automated** — `fastlane/metadata/rating_config.json` encodes every answer as
**No / None**, which the release lanes submit via `app_rating_config_path`,
resolving to **4+**. No objectionable content, no gambling or contests, no
unrestricted web access (the app loads its own bundled editor, not a browser).
On user-generated content the config answers `false`: diagrams are shared only
by explicit link or invite, not published to a public, discoverable feed, so
this is a document tool, not a social UGC platform.

### Category

**Automated** — the release lanes set these via `primary_category` /
`secondary_category`:

- Primary: **Developer Tools** (`DEVELOPER_TOOLS`)
- Secondary: **Education** (`EDUCATION`)

A UML / software-design modeling tool whose audience is engineers, students, and
educators. Independent of the keyword field.

### Export compliance and content rights

**Automated.** `ITSAppUsesNonExemptEncryption` is `false` in `Info.plist` (the
app uses only standard OS-provided HTTPS/TLS), so no encryption question appears
on upload and no annual self-classification report is required. At submit time
the `release_app_store` lane also answers the review questions via
`submission_information` (`export_compliance_uses_encryption: false`,
`content_rights_contains_third_party_content: false`) so submission never stalls.

### Permissions / usage strings

**None required.** The app requests no camera, photo library, location,
contacts, or microphone access. Exports write to the app's own cache directory
and share via the iOS share sheet, so no `NS*UsageDescription` keys are needed.

### Account deletion

**Not applicable / exempt** — Apollon has no account system, so App Store
Guideline 5.1.1(v) (in-app account deletion) does not apply. To delete a shared
diagram, users email the support address with the share link (documented in the
privacy statement and the Support page); shared diagrams also auto-delete after
120 days.

### Sign in with Apple

**Not applicable** — the app offers no third-party or social login, so Sign in
with Apple is not required.

### App Review Information

- **Sign-in required: No** — no demo account is needed.
- **Reviewer notes** are committed at `fastlane/metadata/review_information/notes.txt`
  and uploaded by the release lanes.
- **Contact name, email, and phone** are **automated from CI secrets** (no PII in
  git): set `IOS_APP_REVIEW_FIRST_NAME`, `IOS_APP_REVIEW_LAST_NAME`,
  `IOS_APP_REVIEW_EMAIL`, `IOS_APP_REVIEW_PHONE`. Any unset secret is skipped,
  leaving the existing App Store Connect value.

## Upload paths

TestFlight uploads run automatically after the matching standalone release.
Manually run the `ios-release` workflow only to recover an interrupted upload,
or to use one of these explicit destinations:

- `testflight` for internal testing;
- `app-store-assets` to update metadata and screenshots without a binary; or
- `app-store` to prepare the version from the already-tested TestFlight build
  and, optionally, submit it.

For either App Store destination, choose `raw` or `framed` under **Screenshot
presentation**. The approved official-bezel treatment is the default; choose
`raw` only to upload the untouched Simulator captures instead. From a local
Fastlane invocation, set `APP_STORE_SCREENSHOT_STYLE=raw` to select the raw
masters.

The App Store workflow explicitly targets the package version, so Fastlane can
create that version if it does not exist and cannot silently edit a different
draft. It never rebuilds the App Store binary: when **Submit for review** is
enabled or disabled, enter the exact processed TestFlight build number shown in
App Store Connect. With submission disabled, the workflow prepares the complete
draft and stops. Keep submission disabled until the upgrade/migration test,
privacy answers, review contact, agreements, and screenshots have all been
approved.

## Submission checklist

Automated / already in the repo:

- [x] Bundle id `de.tum.cit.ase.apollon` unchanged (in-place replacement).
- [x] Localized metadata in `fastlane/metadata` (name, subtitle, keywords,
      promo, description, release notes, copyright, marketing/support/privacy
      URLs), validated by `pnpm appstore:metadata:validate`.
- [x] Primary/secondary category (Developer Tools / Education) — set by the lanes.
- [x] Age rating → 4+ from `fastlane/metadata/rating_config.json` — set by the lanes.
- [x] Reviewer notes in `fastlane/metadata/review_information/notes.txt`; contact
      from CI secrets (`IOS_APP_REVIEW_*`).
- [x] App Privacy declaration in `fastlane/app_privacy_details.json`, ready for
      the separately authenticated, deliberate publish step.
- [x] Export compliance (`ITSAppUsesNonExemptEncryption=false` in `Info.plist`)
      and content-rights answers (`submission_information`).
- [x] iPhone + iPad screenshots generated and validated (no alpha, sRGB, exact
      dimensions) by the release lane.
- [x] 1024×1024 marketing icon present in the asset catalog.

One-time setup that can't ride on the API key:

- [ ] App Privacy label: confirm the committed declaration, then run the
      `privacy` lane with an Apple ID session (`FASTLANE_USER` /
      `FASTLANE_SESSION`) or publish the identical answers in App Store Connect.
- [ ] Populate the `IOS_APP_REVIEW_*` CI secrets with the review contact.

Per-release, before enabling **Submit for review**:

- [ ] Docs merged and deployed so the marketing, support, and privacy URLs
      return 200 (the release workflow validates this and stops otherwise).
- [ ] Legacy in-place upgrade/migration verified on a real iPhone and iPad, with
      a diagram backup exported first.
- [ ] Screenshot review artifact inspected at full size.
- [ ] Exact processed TestFlight build number copied into the submission
      workflow input.
- [ ] Version and build number, signing certificate, provisioning profile,
      paid-apps/distribution agreements, and tax/banking status confirmed.
- [ ] DSA trader status set (required for EU distribution).
