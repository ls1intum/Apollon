...
renders the default palette, zoom, layout, and minimap; passing children makes the chrome
...
| `controls`             | `OverlayControlInput[]`       | default chrome                 | Built-in and custom controls to register initially. Omit for defaults, pass `[]` for a bare canvas, or pass descriptors from `paletteControl()`, `zoomControl()`, `layoutControl()`, `miniMapControl()`, `defaultControls()`, or custom controls. See [Overlay controls](/library/api/overlay-controls). |
...
| `fitView(options?)`                            | `({ padding?, duration?, respectInsets? }) => void` | Fit the diagram in view, capped at `maxZoom: 1.0`. Respects reserved [overlay control](/library/api/overlay-controls) insets by default; pass `respectInsets: false` to ignore them. `duration` defaults to `200` ms and becomes `0` when the user prefers reduced motion. |
...
| `arrangeDiagram(options?)`       | `Promise<ArrangeDiagramResult>` | Arrange a flat diagram off-thread and fit moved nodes into view. Pass `fitView: false` to preserve the viewport, fit-view options to customize it, an `AbortSignal` to cancel, or `replaceManualRoutes: true` after confirming replacement of visible manual routes. |

### Automatic arrangement

`arrangeDiagram()` uses Apollon's automatic edge router and rendered label
geometry to choose among deterministic placements. It applies only a materially
clearer result, preserves hidden elements, and returns `{ status: "unchanged" }`
when the bounded search finds no improvement. When visible edges contain
authored bends or pinned endpoints, the safe default returns
`{ status: "confirmation-required", manualRouteCount }` without starting work
or changing the model. After the host confirms, call again with
`replaceManualRoutes: true`; those edges are then scored as automatic routes
and, on success, their manual routing is cleared together with the node
positions in one undo item. Concurrent geometry/model edits return
`{ status: "stale" }` without applying the result. An applied result reports
`movedNodeCount` and `replacedManualRouteCount`; the viewport is fitted only
when at least one node moved. Fitting is local, inset-aware, capped at 100% zoom,
and instantaneous when the user prefers reduced motion. Pass `fitView: false`
to preserve the viewport, or `fitView: { padding, duration, respectInsets }` to
customize it. Viewport changes are not part of the document or undo history.
Cancellation rejects with an
`AbortError`. Operational failures reject with `DiagramLayoutError`; its `code`
is `"timeout"`, `"worker-start"`, `"worker-runtime"`, or `"invalid-result"`. No
failure applies a partial layout. The Worker has a five-second safety timeout.

The production profile supports every flat diagram family, including diagrams
with straight connections, with at most 120 visible nodes and 240 visible edges.
Nested nodes are reported as `unavailable` rather than being partially
rearranged.

```ts no-check
const controller = new AbortController()
const result = await editor.arrangeDiagram({ signal: controller.signal })

const confirmedResult =
  result.status === "confirmation-required"
    ? await editor.arrangeDiagram({
        signal: controller.signal,
        replaceManualRoutes: true,
      })
    : result

if (confirmedResult.status === "applied") {
  persist(editor.model)
}
```
